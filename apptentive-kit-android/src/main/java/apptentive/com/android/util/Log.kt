...
object Log {
    ...
    private const val CHUNK_LOG_MESSAGE_LENGTH =
        3900 // Max length is 4068. 3900 gives room for additional info
    private val logger: Logger = try {
        DependencyProvider.of()
    } catch (e: Exception) {
        Log.w(
            "Apptentive",
            "Failed to get logger from DependencyProvider. Registering and using fallback."
        )
    ...
    @JvmStatic
    fun v(tag: LogTag, format: String, vararg args: Any?) = log(
        LogLevel.Verbose, tag,
        tryFormat(format, *args)
    )

    @JvmStatic
    fun d(tag: LogTag, format: String, vararg args: Any?) = log(
        LogLevel.Debug, tag,
        tryFormat(format, *args)
    )

    @JvmStatic
    fun w(tag: LogTag, format: String, vararg args: Any?) = log(
        LogLevel.Warning, tag,
        tryFormat(format, *args)
    )

    @JvmStatic
    fun e(tag: LogTag, e: Throwable, format: String, vararg args: Any?) = log(
        LogLevel.Error, tag,
        tryFormat(format, *args),
        e
    )
    ...
    fun getMessagesWithTags(message: String, tag: LogTag): List<String> {
        ...
        // thread name
        val threadTag =
            "[${if (logger.isMainQueue()) "Main Queue" else Thread.currentThread().name}]"
