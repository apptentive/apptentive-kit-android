...
internal fun getAlternateTextGravity(scaleType: LayoutOptions): Int {
    return when (scaleType) {
        LayoutOptions.FULL_WIDTH -> Gravity.CENTER
        LayoutOptions.CENTER -> Gravity.CENTER
        LayoutOptions.ALIGN_LEFT -> Gravity.START
        LayoutOptions.ALIGN_RIGHT -> Gravity.END
    }
}
...
internal fun getImageScaleTypeFromConfig(isWiderImage: Boolean, scaleType: LayoutOptions): ImageView.ScaleType = when {
    scaleType == LayoutOptions.FULL_WIDTH || isWiderImage -> ImageView.ScaleType.FIT_XY
    scaleType == LayoutOptions.CENTER -> ImageView.ScaleType.CENTER_INSIDE
    scaleType == LayoutOptions.ALIGN_LEFT -> ImageView.ScaleType.FIT_START
    scaleType == LayoutOptions.ALIGN_RIGHT -> ImageView.ScaleType.FIT_END
