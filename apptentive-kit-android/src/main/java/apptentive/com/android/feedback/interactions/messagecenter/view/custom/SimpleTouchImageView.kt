package apptentive.com.android.feedback.interactions.messagecenter.view.custom
...
/**
 * From the GitHub:
 * TouchImageView extends ImageView and supports all of ImageView’s functionality. In addition,
 * TouchImageView adds pinch zoom, dragging, fling, double tap zoom functionality and other
 * animation polish. The intention is for TouchImageView to mirror as closely as possible the
 * functionality of zoomable images in Gallery apps.
 *
 * Removed some unused functionality and removed non-null modifiers (!!)
 *
...
internal class SimpleTouchImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) :
    ...
    private var floatMatrix = FloatArray(9)
    ...
    private var touchScaleType = ScaleType.FIT_CENTER
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (delayedZoomVariables != null) {
            setZoom(delayedZoomVariables?.scale ?: 0f, delayedZoomVariables?.focusX ?: 0f, delayedZoomVariables?.focusY ?: 0f, delayedZoomVariables?.scaleType)
    ...
    private fun setZoom(scale: Float, focusX: Float, focusY: Float, scaleType: ScaleType?) {
        ...
        if (scaleType != touchScaleType) {
            setScaleType(scaleType ?: ScaleType.MATRIX)
        }
        resetZoom()
     ...
     */
    private fun setZoom(imageSource: SimpleTouchImageView) {
    ...
    private fun orientationMismatch(drawable: Drawable?): Boolean {
        return viewWidth > viewHeight != (drawable?.intrinsicWidth ?: 0) > (drawable?.intrinsicHeight ?: 0)
    }
    ...
    private companion object {
        ...
        private const val SUPER_MAX_MULTIPLIER = 1.5f
